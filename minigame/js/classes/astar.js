...
var astar = {
    ...
    search: function(grid, start, end, diagonal, heuristic) {
        ...
        while(openHeap.size() > 0) {
            ...
            for(var i=0, il = neighbors.length; i < il; i++) {
                ...
                if(neighbor.closed || neighbor.isWall() || neighbor.isFoe()) {
    ...
    neighbors: function(grid, node, diagonals) {
        ...
        if(grid[x-1] && grid[x-1][y]) {
            ret.push(grid[x-1][y]);
			if(grid[x-1][y].isWall()) { west++; }
        }

        // East
        if(grid[x+1] && grid[x+1][y]) {
            ret.push(grid[x+1][y]);
			if(grid[x+1][y].isWall()) { east++; }
        }

        // South
        if(grid[x] && grid[x][y-1]) {
            ret.push(grid[x][y-1]);
			if(grid[x][y-1].isWall()) { south++; }
        }

        // North
        if(grid[x] && grid[x][y+1]) {
            ret.push(grid[x][y+1]);
			if(grid[x][y+1].isWall()) { north++; }
        }
